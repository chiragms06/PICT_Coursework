...
public:
    ...
    Job(int id, string name, int at, int bt, int prio)
    {
        this->id = id;
        this->name = name;
        this->at = at;
...
public:
    ...
    void input()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            cout << "Enter arrival time, burst time and priority of process P" << (i + 1) << " : " << endl;
            cin >> a >> b >> p;
            Job j(i + 1, "P" + to_string(i + 1), a, b, p);
            processes[i] = j;
        }
    }
    ...
    void sjf()
    {
        ...
        while (currentTime <= totalTime)
        {
            while (processes[processCompleted].at <= currentTime && processCompleted < n)
            {
                readyQueue.push_back(processes[processCompleted]);
                processCompleted++;
            }
    ...
    void priority()
    {
        ...
        while (currentTime <= totalTime)
        {
            ...
            // Sort the readyQueue excluding the first process
            if (readyQueue.size() > 1)
            {
                sort(readyQueue.begin() + 1, readyQueue.end(), [](const Job &j1, const Job &j2)
